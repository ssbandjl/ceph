...
CDentry *CDir::lookup_exact_snap(std::string_view name, snapid_t last) {
  dout(20) << __FFL__ << " (" << last << ", '" << name << "')" << dendl;
...
{
  dout(12) << __FFL__ << " " << *dn << " remote " << ino << dendl;
...
{
  if (dn->get_linkage()->is_primary()) {
    dout(12) << __FFL__ << " " << *dn << " " << *dn->get_linkage()->get_inode() << dendl;
  } else {
    dout(12) << __FFL__ << " " << *dn << dendl;
  }
...
void CDir::try_remove_unlinked_dn(CDentry *dn)
{
  ...
  // was the dn new?
  if (dn->is_new()) {
    dout(10) << __FFL__ << " " << *dn << " in " << *this << dendl;
    if (dn->is_dirty())
      dn->mark_clean();
    remove_dentry(dn);
...
void CDir::resync_accounted_fragstat()
{
  ...
  if (pf->accounted_fragstat.version != pi->dirstat.version) {
    ...
    dout(10) << __FFL__ << " " << pf->accounted_fragstat << " -> " << pf->fragstat << dendl;
...
void CDir::resync_accounted_rstat()
{
  ...
  if (pf->accounted_rstat.version != pi->rstat.version) {
    ...
    dout(10) << __FFL__ << " " << pf->accounted_rstat << " -> " << pf->rstat << dendl;
...
{
  dout(11) << __FFL__ << " mask " << hex << mask << dec << " result " << result << " on " << *this << dendl;
...
void CDir::pop_and_dirty_projected_fnode(LogSegment *ls)
{
  ...
  dout(15) << __FFL__ << " " << &front << " v" << front.version << dendl;
  fnode = front;
  _mark_dirty(ls);
...
version_t CDir::pre_dirty(version_t min)
{
  ...
  ++projected_version;
  dout(10) << __FFL__ << " " << projected_version << dendl;
...
{
  if (!state_test(STATE_DIRTY)) {
    dout(10) << __FFL__ << " (was clean) " << *this << " version " << get_version() << dendl;
    _set_dirty_flag();
    ceph_assert(ls);
  } else {
    dout(10) << __FFL__ << " (already dirty) " << *this << " version " << get_version() << dendl;
  }
  if (ls) {
...
{
  dout(10) << __FFL__ << " " << *this << " version " << get_version() << dendl;
  if (state_test(STATE_DIRTY)) {
...
void CDir::adjust_nested_auth_pins(int dirinc, void *by)
{
  ...
  dout(15) << __FFL__ << " " << dirinc << " on " << *this
	   << " by " << by << " count now "
...
void CDir::maybe_finish_freeze()
{
  ...
  if (freeze_tree_state) {
    ...
    if (!is_subtree_root() && inode->is_frozen()) {
      dout(10) << __FFL__ << " !subtree root and frozen inode, waiting for unfreeze on " << inode << dendl;
      // retake an auth_pin...
      auth_pin(inode);
...
int CDir::scrub_dentry_next(MDSContext *cb, CDentry **dnout)
{
  ...
  int rval = _next_dentry_on_set(scrub_infop->directories_to_scrub, true,
                                 cb, dnout);
  if (rval == 0) {
    dout(20) << __FFL__ << " inserted to directories scrubbing: "
      << *dnout << dendl;
    ...
    if (rval == 0) {
      dout(20) << __FFL__ << " inserted to others scrubbing: "
        << *dnout << dendl;
...
{
  dout(20) << __FFL__ << " on dn " << *dn << dendl;
  ceph_assert(scrub_infop && scrub_infop->directory_scrubbing);
