...
void ProtocolV1::fault() {
  ...
  if (state == CLOSED || state == NONE) {
    ldout(cct, 10) << __FFL__ << " connection is already closed" << dendl;
    return;
  }

  if (connection->policy.lossy && state != START_CONNECT &&
      state != CONNECTING) {
    ldout(cct, 1) << __FFL__ << " on lossy channel, failing" << dendl;
    stop();
  ...
  if (!once_ready && out_q.empty() && state >= START_ACCEPT &&
    ...
    ldout(cct, 10) << __FFL__ << " with nothing to send and in the half "
  ...
  if (connection->policy.standby && out_q.empty() && !keepalive &&
      state != WAIT) {
    ldout(cct, 10) << __FFL__ << " with nothing to send, going to standby"
                   << dendl;
    state = STANDBY;
    connection->write_lock.unlock();
    return;
  ...
  if ((state >= START_CONNECT && state <= CONNECTING_SEND_CONNECT_MSG) ||
    ...
    ldout(cct, 10) << __FFL__ << " waiting " << backoff << dendl;
    // woke up again;
    ...
    if (connection->policy.server) {
      ldout(cct, 0) << __FFL__ << " server, going to standby" << dendl;
      state = STANDBY;
    } else {
      ldout(cct, 0) << __FFL__ << " initiating reconnect" << dendl;
      connect_seq++;
      global_seq = messenger->get_global_seq();
      state = START_CONNECT;
      connection->state = AsyncConnection::STATE_CONNECTING;
...
void ProtocolV1::send_message(Message *m) {
  ...
  if (can_fast_prepare &&
    ...
    bl.clear();
    m->clear_payload();
    ldout(cct, 5) << __FFL__ << " clear encoded buffer previous " << f
                  << " != " << connection->get_features() << dendl;
  }
  if (can_write == WriteStatus::CLOSED) {
    ldout(cct, 10) << __FFL__ << " connection closed."
    ...
    ldout(cct, 15) << __FFL__ << " inline write is denied, reschedule m=" << m
                   << dendl;
...
void ProtocolV1::prepare_send_message(uint64_t features, Message *m,
                                      bufferlist &bl) {
  ldout(cct, 20) << __FFL__ << " m " << *m << dendl;

  // associate message with Connection (for benefit of encode_payload)
  ldout(cct, 20) << __FFL__ << (m->empty_payload() ? " encoding features " : " half-reencoding features ")
		 << features << " " << m  << " " << *m << dendl;
  ...
  if (can_write == WriteStatus::CANWRITE) {
    ...
    do {
      ...
      if (r == 0) {
        ...
        ldout(cct, 1) << __FFL__ << " send msg failed" << dendl;
        break;
      } else if (r > 0) {
    ...
    if (r == 0) {
      ...
      if (left) {
        ...
        ldout(cct, 10) << __FFL__ << " try send msg ack, acked " << left
                       << " messages" << dendl;
        ack_left -= left;
        left = ack_left;
    ...
    if (state == STANDBY && !connection->policy.server && is_queued()) {
      ldout(cct, 10) << __FFL__ << " policy.server is false" << dendl;
      connection->_connect();
    } else if (connection->cs && state != NONE && state != CLOSED &&
               state != START_CONNECT) {
      r = connection->_try_send();
      if (r < 0) {
        ldout(cct, 1) << __FFL__ << " send outcoming bl failed" << dendl;
        connection->write_lock.unlock();
        fault();
        connection->lock.unlock();
        return;
...
CtPtr ProtocolV1::handle_message(char *buffer, int r) {
  ...
  if (r < 0) {
    ldout(cct, 1) << __FFL__ << " read tag failed" << dendl;
    return _fault();
  }

  char tag = buffer[0];
  ldout(cct, 20) << __FFL__ << " process tag " << (int)tag << dendl;

  if (tag == CEPH_MSGR_TAG_KEEPALIVE) {
    ldout(cct, 20) << __FFL__ << " got KEEPALIVE" << dendl;
    ...
    ldout(cct, 20) << __FFL__ << " begin MSG" << dendl;
    return READ(sizeof(ceph_msg_header), handle_message_header);
  } else if (tag == CEPH_MSGR_TAG_CLOSE) {
    ldout(cct, 20) << __FFL__ << " got CLOSE" << dendl;
    stop();
  } else {
    ldout(cct, 0) << __FFL__ << " bad tag " << (int)tag << dendl;
    return _fault();
  }
  return nullptr;
}
...
CtPtr ProtocolV1::handle_keepalive2(char *buffer, int r) {
  ...
  if (r < 0) {
    ldout(cct, 1) << __FFL__ << " read keeplive timespec failed" << dendl;
    return _fault();
  }

  ldout(cct, 30) << __FFL__ << " got KEEPALIVE2 tag ..." << dendl;
  ...
  ldout(cct, 20) << __FFL__ << " got KEEPALIVE2 " << kp_t << dendl;
  connection->set_last_keepalive(ceph_clock_now());
...
CtPtr ProtocolV1::handle_keepalive2_ack(char *buffer, int r) {
  ...
  ldout(cct, 20) << __FFL__ << " got KEEPALIVE_ACK" << dendl;
...
CtPtr ProtocolV1::handle_tag_ack(char *buffer, int r) {
  ...
  if (r < 0) {
    ldout(cct, 1) << __FFL__ << " read ack seq failed" << dendl;
    return _fault();
  }

  ceph_le64 seq;
  seq = *(ceph_le64 *)buffer;
  ldout(cct, 20) << __FFL__ << " got ACK" << dendl;

  ldout(cct, 15) << __FFL__ << " got ack seq " << seq << dendl;
  // trim sent list
  ...
  while (!sent.empty() && sent.front()->get_seq() <= seq && i < max_pending) {
    ...
    sent.pop_front();
    pending[i++] = m;
    ldout(cct, 10) << __FFL__ << " got ack seq " << seq
                   << " >= " << m->get_seq() << " on " << m << " " << *m
                   << dendl;
  }
  connection->write_lock.unlock();
...
CtPtr ProtocolV1::handle_message_header(char *buffer, int r) {
  ...
  if (r < 0) {
    ldout(cct, 1) << __FFL__ << " read message header failed" << dendl;
    return _fault();
  }

  ldout(cct, 20) << __FFL__ << " got MSG header" << dendl;

  current_header = *((ceph_msg_header *)buffer);

  ldout(cct, 20) << __FFL__ << " got envelope type=" << current_header.type << " src "
  ...
  if (messenger->crcflags & MSG_CRC_HEADER) {
    ...
    if (header_crc != current_header.crc) {
      ldout(cct, 0) << __FFL__ << " got bad header crc " << header_crc
...
CtPtr ProtocolV1::throttle_message() {
  ...
  if (connection->policy.throttler_messages) {
    ...
    if (!connection->policy.throttler_messages->get_or_fail()) {
      ldout(cct, 10) << __FFL__ << " wants 1 message from policy throttle "
                     << connection->policy.throttler_messages->get_current()
                     << "/" << connection->policy.throttler_messages->get_max()
...
CtPtr ProtocolV1::throttle_bytes() {
  ...
  if (cur_msg_size) {
    if (connection->policy.throttler_bytes) {
      ldout(cct, 10) << __FFL__ << " wants " << cur_msg_size
...
CtPtr ProtocolV1::handle_message_front(char *buffer, int r) {
  ...
  if (r < 0) {
    ldout(cct, 1) << __FFL__ << " read message front failed" << dendl;
    return _fault();
  }

  ldout(cct, 20) << __FFL__ << " got front " << front.length() << dendl;
...
CtPtr ProtocolV1::handle_message_middle(char *buffer, int r) {
  ...
  if (r < 0) {
    ldout(cct, 1) << __FFL__ << " read message middle failed" << dendl;
    return _fault();
  }

  ldout(cct, 20) << __FFL__ << " got middle " << middle.length() << dendl;
...
CtPtr ProtocolV1::read_message_data_prepare() {
  ...
  if (data_len) {
    ...
    if (p != connection->rx_buffers.end()) {
      ldout(cct, 10) << __FFL__ << " seleting rx buffer v " << p->second.second
      ...
      ldout(cct, 20) << __FFL__ << " allocating new rx buffer at offset "
                     << data_off << dendl;
...
CtPtr ProtocolV1::read_message_data() {
  ldout(cct, 20) << __FFL__ << " msg_left=" << msg_left << dendl;
...
CtPtr ProtocolV1::handle_message_data(char *buffer, int r) {
  ...
  if (r < 0) {
    ldout(cct, 1) << __FFL__ << " read data error " << dendl;
    return _fault();
  }
...
CtPtr ProtocolV1::handle_message_footer(char *buffer, int r) {
  ...
  if (r < 0) {
    ldout(cct, 1) << __FFL__ << " read footer data error " << dendl;
    return _fault();
  }
  ...
  ldout(cct, 10) << __FFL__ << " aborted = " << aborted << dendl;
  if (aborted) {
    ldout(cct, 0) << __FFL__ << " got " << front.length() << " + "
                  << middle.length() << " + " << data.length()
                  << " byte message.. ABORTED" << dendl;
    return _fault();
  }

  ldout(cct, 20) << __FFL__ << " got " << front.length() << " + "
  ...
  if (!message) {
    ldout(cct, 1) << __FFL__ << " decode message failed " << dendl;
    return _fault();
  }
  ...
  if (session_security.get() == NULL) {
    ldout(cct, 10) << __FFL__ << " no session security set" << dendl;
  } else {
    if (session_security->check_message_signature(message)) {
      ldout(cct, 0) << __FFL__ << " Signature check failed" << dendl;
      message->put();
      return _fault();
    }
  }
  ...
  if (message->get_seq() <= cur_seq) {
    ldout(cct, 0) << __FFL__ << " got old message " << message->get_seq()
  ...
  if (message->get_seq() > cur_seq + 1) {
    ldout(cct, 0) << __FFL__ << " missed message?  skipped from seq "
  ...
  if (connection->is_blackhole()) {
    ldout(cct, 10) << __FFL__ << " blackhole " << *message << dendl;
    message->put();
    goto out;
  }
...
void ProtocolV1::session_reset() {
  ldout(cct, 10) << __FFL__ << " started" << dendl;
...
void ProtocolV1::randomize_out_seq() {
  if (connection->get_features() & CEPH_FEATURE_MSG_AUTH) {
    ...
    ldout(cct, 10) << __FFL__ << " randomize_out_seq " << rand_seq << dendl;
    out_seq = rand_seq;
  } else {
...
ssize_t ProtocolV1::write_message(Message *m, bufferlist &bl, bool more) {
  ...
  if (session_security.get() == NULL) {
    ldout(cct, 20) << __FFL__ << " no session security" << dendl;
  } else {
    if (session_security->sign_message(m)) {
      ldout(cct, 20) << __FFL__ << " failed to sign m=" << m
                     << "): sig = " << footer.sig << dendl;
    } else {
      ldout(cct, 20) << __FFL__ << " signed m=" << m
                     << "): sig = " << footer.sig << dendl;
    }
  }
  ...
  ldout(cct, 20) << __FFL__ << " sending message type=" << header.type
...
void ProtocolV1::requeue_sent() {
  ...
  while (!sent.empty()) {
    Message *m = sent.back();
    sent.pop_back();
    ldout(cct, 10) << __FFL__ << " " << *m << " for resend "
                   << " (" << m->get_seq() << ")" << dendl;
    m->clear_payload();
...
uint64_t ProtocolV1::discard_requeued_up_to(uint64_t out_seq, uint64_t seq) {
  ldout(cct, 10) << __FFL__ << " " << seq << dendl;
  std::lock_guard<std::mutex> l(connection->write_lock);
  ...
  while (!rq.empty()) {
    ...
    ldout(cct, 10) << __FFL__ << " " << *(p.second) << " for resend seq "
...
void ProtocolV1::discard_out_queue() {
  ...
  for (list<Message *>::iterator p = sent.begin(); p != sent.end(); ++p) {
    ldout(cct, 20) << __FFL__ << " discard " << *p << dendl;
    (*p)->put();
  }
  sent.clear();
  for (map<int, list<pair<bufferlist, Message *> > >::iterator p =
    ...
    for (list<pair<bufferlist, Message *> >::iterator r = p->second.begin();
         r != p->second.end(); ++r) {
      ldout(cct, 20) << __FFL__ << " discard " << r->second << dendl;
      r->second->put();
    }
  }
  out_q.clear();
...
void ProtocolV1::reset_recv_state()
{
  ...
  if (state > THROTTLE_MESSAGE && state <= READ_FOOTER_AND_DISPATCH &&
    ...
    ldout(cct, 10) << __FFL__ << " releasing " << 1
  ...
  if (state > THROTTLE_BYTES && state <= READ_FOOTER_AND_DISPATCH) {
    if (connection->policy.throttler_bytes) {
      ldout(cct, 10) << __FFL__ << " releasing " << cur_msg_size
  ...
  if (state > THROTTLE_DISPATCH_QUEUE && state <= READ_FOOTER_AND_DISPATCH) {
    ldout(cct, 10)
        << __FFL__ << " releasing " << cur_msg_size
...
CtPtr ProtocolV1::handle_client_banner_write(int r) {
  ...
  if (r < 0) {
    ldout(cct, 1) << __FFL__ << " write client banner failed" << dendl;
    return _fault();
  }
  ldout(cct, 10) << __FFL__ << " connect write banner done: "
...
CtPtr ProtocolV1::handle_server_banner_and_identify(char *buffer, int r) {
  ...
  if (r < 0) {
    ldout(cct, 1) << __FFL__ << " read banner and identify addresses failed"
                  << dendl;
    return _fault();
  }
  ...
  if (memcmp(buffer, CEPH_BANNER, banner_len)) {
    ldout(cct, 0) << __FFL__ << " connect protocol error (bad banner) on peer "
                  << connection->get_peer_addr() << dendl;
    return _fault();
  }
  ...
  try {
    ...
    lderr(cct) << __FFL__ << " decode peer addr failed " << dendl;
    return _fault();
  }
  ldout(cct, 20) << __FFL__ << " connect read peer addr " << paddr
  ...
  if (peer_addr != paddr) {
    if (paddr.is_blank_ip() && peer_addr.get_port() == paddr.get_port() &&
      ...
      ldout(cct, 0) << __FFL__ << " connect claims to be " << paddr << " not "
                    << peer_addr << " - presumably this is the same node!"
                    << dendl;
    } else {
      ldout(cct, 10) << __FFL__ << " connect claims to be " << paddr << " not "
                     << peer_addr << dendl;
      return _fault();
    }
  }

  ldout(cct, 20) << __FFL__ << " connect peer addr for me is "
  ...
  if (messenger->get_myaddrs().empty() ||
    ...
    if (cct->_conf->ms_learn_addr_from_peer) {
      ldout(cct, 1) << __FFL__ << " peer " << connection->target_addr
		    << " says I am " << peer_addr_for_me << " (socket says "
		    << (sockaddr*)&ss << ")" << dendl;
      a = peer_addr_for_me;
    } else {
      ldout(cct, 1) << __FFL__ << " socket to  " << connection->target_addr
	...
	ldout(cct, 10) << __FFL__ << " sleep for "
...
CtPtr ProtocolV1::handle_my_addr_write(int r) {
  ...
  if (r < 0) {
    ldout(cct, 2) << __FFL__ << " connect couldn't write my addr, "
                  << cpp_strerror(r) << dendl;
    return _fault();
  }
  ldout(cct, 10) << __FFL__ << " connect sent my addr "
...
CtPtr ProtocolV1::send_connect_message()
{
  ...
  if (connection->peer_type != CEPH_ENTITY_TYPE_MON ||
    ...
    if (authorizer_more.length()) {
      ldout(cct,10) << __FFL__ << " using augmented (challenge) auth payload"
		    << dendl;
	...
	ldout(cct, 1) << __FFL__ << " state changed!" << dendl;
	return _fault();
      }
    }
  }
  ...
  ldout(cct, 10) << __FFL__ << " connect sending gseq=" << global_seq
                 << " cseq=" << connect_seq
...
CtPtr ProtocolV1::handle_connect_message_write(int r) {
  ...
  if (r < 0) {
    ldout(cct, 2) << __FFL__ << " connect couldn't send reply "
                  << cpp_strerror(r) << dendl;
    return _fault();
  }
...
CtPtr ProtocolV1::handle_connect_reply_1(char *buffer, int r) {
  ...
  if (r < 0) {
    ldout(cct, 1) << __FFL__ << " read connect reply failed" << dendl;
    return _fault();
  }

  connect_reply = *((ceph_msg_connect_reply *)buffer);

  ldout(cct, 20) << __FFL__ << " connect got reply tag "
...
CtPtr ProtocolV1::handle_connect_reply_auth(char *buffer, int r) {
  ...
  if (r < 0) {
    ldout(cct, 1) << __FFL__ << " read connect reply authorizer failed"
                  << dendl;
    return _fault();
  }
...
CtPtr ProtocolV1::handle_connect_reply_2() {
  ...
  if (connect_reply.tag == CEPH_MSGR_TAG_FEATURES) {
    ldout(cct, 0) << __FFL__ << " connect protocol feature mismatch, my "
  ...
  if (connect_reply.tag == CEPH_MSGR_TAG_BADPROTOVER) {
    ldout(cct, 0) << __FFL__ << " connect protocol version mismatch, my "
                  << messenger->get_proto_version(connection->peer_type, true)
  ...
  if (connect_reply.tag == CEPH_MSGR_TAG_BADAUTHORIZER) {
    ldout(cct, 0) << __FFL__ << " connect got BADAUTHORIZER" << dendl;
    authorizer_more.clear();
    return _fault();
  }

  if (connect_reply.tag == CEPH_MSGR_TAG_RESETSESSION) {
    ldout(cct, 0) << __FFL__ << " connect got RESETSESSION" << dendl;
    session_reset();
    connect_seq = 0;
  ...
  if (connect_reply.tag == CEPH_MSGR_TAG_RETRY_GLOBAL) {
    ...
    ldout(cct, 5) << __FFL__ << " connect got RETRY_GLOBAL "
  ...
  if (connect_reply.tag == CEPH_MSGR_TAG_RETRY_SESSION) {
    ...
    ldout(cct, 5) << __FFL__ << " connect got RETRY_SESSION " << connect_seq
  ...
  if (connect_reply.tag == CEPH_MSGR_TAG_WAIT) {
    ldout(cct, 1) << __FFL__ << " connect got WAIT (connection race)" << dendl;
    state = WAIT;
    return _fault();
  }
  ...
  if (feat_missing) {
    ldout(cct, 1) << __FFL__ << " missing required features " << std::hex
  ...
  if (connect_reply.tag == CEPH_MSGR_TAG_READY) {
    ldout(cct, 10) << __FFL__ << " got CEPH_MSGR_TAG_READY " << dendl;
  }
...
CtPtr ProtocolV1::handle_ack_seq(char *buffer, int r) {
  ...
  if (r < 0) {
    ldout(cct, 1) << __FFL__ << " read connect ack seq failed" << dendl;
    return _fault();
  }
  ...
  ldout(cct, 2) << __FFL__ << " got newly_acked_seq " << newly_acked_seq
...
CtPtr ProtocolV1::handle_in_seq_write(int r) {
  ...
  if (r < 0) {
    ldout(cct, 10) << __FFL__ << " failed to send in_seq " << dendl;
    return _fault();
  }

  ldout(cct, 10) << __FFL__ << " send in_seq done " << dendl;
...
CtPtr ProtocolV1::client_ready() {
  ...
  ldout(cct, 10) << __FFL__ << " connect success " << connect_seq
  ...
  if (auth_meta->authorizer) {
    ldout(cct, 10) << __FFL__ << " setting up session_security with auth "
    ...
    ldout(cct, 10) << __FFL__ << " no authorizer, clearing session_security"
		   << dendl;
    session_security.reset();
  }
...
CtPtr ProtocolV1::send_server_banner() {
  ...
  ldout(cct, 1) << __FFL__ << " sd=" << connection->cs.fd()
		<< " legacy " << legacy
...
CtPtr ProtocolV1::handle_server_banner_write(int r) {
  ...
  ldout(cct, 10) << __FFL__ << " write banner and addr done: "
                 << connection->get_peer_addr() << dendl;
...
CtPtr ProtocolV1::handle_client_banner(char *buffer, int r) {
  ...
  if (r < 0) {
    ldout(cct, 1) << __FFL__ << " read peer banner and addr failed" << dendl;
    return _fault();
  }

  if (memcmp(buffer, CEPH_BANNER, strlen(CEPH_BANNER))) {
    ldout(cct, 1) << __FFL__ << " accept peer sent bad banner '" << buffer
  ...
  try {
    ...
    lderr(cct) << __FFL__ << " decode peer_addr failed " << dendl;
    return _fault();
  }

  ldout(cct, 10) << __FFL__ << " accept peer addr is " << peer_addr << dendl;
  if (peer_addr.is_blank_ip()) {
    ...
    ldout(cct, 0) << __FFL__ << " accept peer addr is really " << peer_addr
...
CtPtr ProtocolV1::handle_connect_message_1(char *buffer, int r) {
  ...
  if (r < 0) {
    ldout(cct, 1) << __FFL__ << " read connect msg failed" << dendl;
    return _fault();
  }
...
CtPtr ProtocolV1::handle_connect_message_auth(char *buffer, int r) {
  ...
  if (r < 0) {
    ldout(cct, 1) << __FFL__ << " read connect authorizer failed" << dendl;
    return _fault();
  }
...
CtPtr ProtocolV1::handle_connect_message_2() {
  ...
  ldout(cct, 20) << __FFL__ << " accept got peer connect_seq "
  ...
  ldout(cct, 10) << __FFL__ << " accept of host_type " << connect_msg.host_type
  ...
  // mismatch?
  ldout(cct, 10) << __FFL__ << " accept my proto " << reply.protocol_version
  ...
  if (feat_missing) {
    ldout(cct, 1) << __FFL__ << " peer missing required features " << std::hex
                  << feat_missing << std::dec << dendl;
  ...
  connection->lock.unlock();
  ldout(cct,10) << __FFL__ << " authorizor_protocol "
  ...
  if (r < 0) {
    ...
    ldout(cct, 0) << __FFL__ << ": got bad authorizer, auth_reply_len="
  ...
  if (r == 0) {
    ...
    ldout(cct, 10) << __FFL__ << ": challenging authorizer" << dendl;
  ...
  ldout(cct, 10) << __FFL__ << " accept setting up session_security." << dendl;
  ...
  if (connection->policy.server &&
    ...
    // new session
    ldout(cct, 10) << __FFL__ << " accept new session" << dendl;
    connection->lock.lock();
  ...
  if (existing && existing->protocol->proto_type != 1) {
    ldout(cct,1) << __FFL__ << " existing " << existing << " proto "
  ...
  if (existing) {
    ...
    ldout(cct,10) << __FFL__ << " existing=" << existing << " exproto="
    ...
    if (exproto->state == CLOSED) {
      ldout(cct, 1) << __FFL__ << " existing " << existing
    ...
    if (connect_msg.global_seq < exproto->peer_global_seq) {
      ldout(cct, 10) << __FFL__ << " accept existing " << existing << ".gseq "
    ...
    ldout(cct, 1) << __FFL__ << " accept connect_seq "
    ...
    if (connect_msg.connect_seq < exproto->connect_seq) {
      ...
      ldout(cct, 10) << __FFL__ << " accept existing " << existing << ".cseq "
    ...
    if (connect_msg.connect_seq == exproto->connect_seq) {
      ...
      if (exproto->state == OPENED || exproto->state == STANDBY) {
        ldout(cct, 10) << __FFL__ << " accept connection race, existing "
      ...
      if (connection->peer_addrs->legacy_addr() < messenger->get_myaddr_legacy() ||
        ...
        ldout(messenger->cct, 10)
            << __FFL__ << " accept connection race, existing " << existing
    ...
    if (connection->policy.resetcheck &&  // RESETSESSION only used by servers;
      ...
      ldout(cct, 0) << __FFL__ << " accept we reset (peer sent cseq "
    ...
    // reconnect
    ldout(cct, 10) << __FFL__ << " accept peer sent cseq "
...
CtPtr ProtocolV1::send_connect_message_reply(char tag,
  ...
  ldout(cct, 10) << __FFL__ << " reply features 0x" << std::hex
...
CtPtr ProtocolV1::replace(const AsyncConnectionRef& existing,
                          ceph_msg_connect_reply &reply,
                          bufferlist &authorizer_reply) {
  ldout(cct, 10) << __FFL__ << " accept replacing " << existing << dendl;

  connection->inject_delay();
  if (existing->policy.lossy) {
    // disconnect from the Connection
    ldout(cct, 1) << __FFL__ << " replacing on lossy channel, failing existing"
                  << dendl;
    ...
    ldout(messenger->cct, 1)
        << __FFL__ << " stop myself to swap existing" << dendl;
...
CtPtr ProtocolV1::open(ceph_msg_connect_reply &reply,
  ...
  ldout(cct, 10) << __FFL__ << " accept success, connect_seq = " << connect_seq
  ...
  ldout(cct, 10) << __FFL__ << " accept features "
                 << connection->get_features()
		 << " authorizer_protocol "
  ...
  replacing = false;
  if (r < 0) {
    ldout(cct, 1) << __FFL__ << " existing race replacing process for addr = "
...
CtPtr ProtocolV1::handle_ready_connect_message_reply_write(int r) {
  ...
  if (r < 0) {
    ldout(cct, 1) << __FFL__ << " write ready connect message reply failed"
                  << dendl;
    return _fault();
  }
...
CtPtr ProtocolV1::handle_seq(char *buffer, int r) {
  ...
  ldout(cct, 2) << __FFL__ << " accept get newly_acked_seq " << newly_acked_seq
                << dendl;
  out_seq = discard_requeued_up_to(out_seq, newly_acked_seq);
...
CtPtr ProtocolV1::server_ready() {
  ldout(cct, 20) << __FFL__ << " session_security is "
		 << session_security
		 << dendl;

  ldout(cct, 20) << __FFL__ << " accept done" << dendl;
  // FIPS zeroization audit 20191115: this memset is not security related.
  memset(&connect_msg, 0, sizeof(connect_msg));
